...
def _guess_meeting_dir(session_dir: Path) -> Optional[Path]:
    """
    Best-effort inference of meeting root from a session dir.
...
def _load_whiteboard_items(session_dir: Path) -> List[Dict]:
    """
    Load whiteboard items (if any) and attach `src_rel` for <img src="...">,
    relative to the session_dir where report.html lives.
    """
    meeting_dir = _guess_meeting_dir(session_dir) or session_dir
        ...
        with items_path.open("r", encoding="utf-8") as f:
            for ln in f:
                ...
                if not s:
                    continue
                row = json.loads(s)
                rel_from_meeting = row.get("path")
                if not rel_from_meeting:
                    continue
                frame_abs = meeting_dir / rel_from_meeting
                # relative path from session dir → report.html can load image
                rel = os.path.relpath(frame_abs, session_dir)
                row["src_rel"] = rel.replace("\\", "/")
                items.append(row)
    except Exception as e:
        log.warning("Could not load whiteboard items from %s: %s", items_path, e)
        return []
...
def render_report(session_dir: str | Path, auto_open: bool = False) -> Path:
    ...
    if meta_p.exists():
        try:
            ...
            log.warning("Could not read meta.json (%s): %s", meta_p, e)
            meta = {}
    ...
    for idx, r in enumerate(rows):
        ...
        if by_seg.get(idx):
            words_html = " ".join(
                f'<span class="w" data-start="{float(w.get("start",0.0)):.3f}" '
                f'data-end="{float(w.get("end",0.0)):.3f}">{html.escape(str(w.get("word","")))}</span>'
                for w in by_seg[idx]
            )
    ...
    # whiteboard items (if any)
    wb_items = _load_whiteboard_items(sdir)
    if wb_items:
        cards = []
        for it in wb_items:
            src = html.escape(it.get("src_rel", ""))
            ocr = (it.get("ocr_text") or "").strip()
            ts_hint = it.get("timestamp_hint")
            ts_label = f"~{ts_hint:.1f}s" if isinstance(ts_hint, (int, float)) else ""
            cards.append(
                f"""
            <div class="wb-card">
              <div class="wb-img-wrap">
                <img src="{src}" loading="lazy" class="wb-img" alt="whiteboard frame"/>
              </div>
              <div class="wb-meta">
                <div class="wb-ts">{html.escape(ts_label)}</div>
                <div class="wb-ocr">{html.escape(ocr) if ocr else "(no OCR text)"}</div>
              </div>
            </div>"""
            )
        whiteboard_html = f'<div class="wb-grid">{"".join(cards)}</div>'
    else:
        whiteboard_html = "<p class='small'>(No whiteboard frames found)</p>"
