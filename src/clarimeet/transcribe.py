...
def _write_meta(
    ...
    meta = {
        ...
        "config": config,  # includes fp16/device/temperature/seed/etc.
    }
    with p.open("w", encoding="utf-8") as f:
...
def transcribe_file(
    ...
    # Select device & fp16 automatically
    ...
    # Options (enable fp16 only on GPU)
    options = dict(
        language=language,
    ...
    meta = _write_meta(
        ...
        # record device/fp16/seed in config (not used by whisper, but useful metadata)
        config={**options, "device": device, "fp16": use_fp16, "seed": seed},
    )
