...
export const GoogleSignIn: React.FC<GoogleSignInProps> = ({
  ...
  useEffect(() => {
    ...
    window.onGoogleSignIn = handleCredentialResponse
...
export const CustomGoogleSignIn: React.FC<GoogleSignInProps> = ({
  ...
  const handleSignIn = () => {
    ...
    if (!clientId || clientId === 'your-google-client-id') {
      console.error('Google Client ID not configured. Please set NEXT_PUBLIC_GOOGLE_CLIENT_ID in your .env.local file')
      alert('Google Sign-In not configured. Please set up your Google Client ID.')
      if (onError) {
        onError(new Error('Google Client ID not configured'))
      }
      return
  ...
  return (
    <button
      ...
      className={`flex items-center justify-center gap-3 bg-black/60 backdrop-blur-xl hover:bg-black/70 text-white font-medium py-2 px-4 rounded-lg border border-cyan-400/30 transition-all duration-200 hover:border-cyan-400/50 hover:shadow-lg ${className}`}
    >
